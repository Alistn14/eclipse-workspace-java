...
public class RootHandler implements HttpHandler {
	public void handle(HttpExchange he) throws IOException {
		...
		try {
			// respuesta al cliente para peticiones GET (descarga de la pagina)
			...
			// respuesta al cliente para peticiones GET (envio de los datos del formulario)
			else if (he.getRequestMethod().equalsIgnoreCase("POST")) {
				// obtencion de los datos del cuerpo
				...
				in.close();
				// obtencion del usuario y password hasheado
				...
				// si la autenticacion es correcta, se devuelve ok. En caso contrario, se devuelve error
				String response = "error";
				if (resultados.next()) {
					response = "ok";
				}
				// envia respuesta al cliente y cierra la conexion
				he.sendResponseHeaders(200, response.length());
				OutputStream os = he.getResponseBody();
				os.write(response.getBytes());
				os.close();
	...
	// extrae los valores del formulario enviados en el cuerpo de la peticion POST
